...
import { Check, CheckCircle2, Copy, Eye, EyeOff, Lock, Mail, User, X } from 'lucide-react';
...
export default function LoginPage({ onLoginSuccess }) {
  ...
  return <div className="flex min-h-screen items-center justify-center bg-[#f7f5f0] px-4 py-6 font-sans text-stone-700 dark:bg-[#20241f] dark:text-stone-200 sm:px-6 sm:py-8 md:px-8">
    <div className="relative w-full max-w-md space-y-6 rounded-[18px] bg-[#fffefa] p-5 neu-flat dark:bg-[#292d28] sm:p-8 md:p-10">
      <div className="flex justify-center"><BrandLogo showName /></div>
      <div className="space-y-1 text-center"><h1 className="text-2xl font-bold tracking-tight text-slate-800 dark:text-slate-100">{mode === 'signin' ? 'Welcome back' : 'Create an account'}</h1><p className="text-xs text-slate-400">{mode === 'signin' ? 'Please sign in to continue' : 'Enter your B.Tech scholar details'}</p></div>
