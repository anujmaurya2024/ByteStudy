...
public class AcademicService {
    ...
    public User activateEndSemSubscription(User user) {
        ...
        boolean alreadyActive = subscriptionRepo.findByUserOrderByCreatedAtDesc(user).stream()
            .anyMatch(subscription -> subscription.isActive(java.time.Instant.now()));
        if (!alreadyActive) {
            subscriptionRepo.save(com.bytepath.model.Subscription.builder()
                .user(user).plan("END_SEM").status("ACTIVE")
                .provider("PENDING_PAYMENT_VERIFICATION")
                .startsAt(java.time.Instant.now()).build());
        }
        return userRepo.save(user);
    }
